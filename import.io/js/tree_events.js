...
function delete_mouseover_popup(d3){
  return function (d) {
    if(d.repeated && d.repeated.length > 0 && !d3.select("#mouseover").empty()){
...
function mouseover(d3) {
  return function (d) {
    if(d.repeated && d.repeated.length > 0 && d3.select("#mouseover").empty()){
      ...
      .append("p")
      .append("center")
      .text(d.name + " is repeated in " + d.repeated.join(", "))
    }
  }
}
...
function update_color(d) {
  ...
  if(d.repeated && d.repeated.length > 0 && children_count  > 15) return "orange";
  if(d.repeated && d.repeated.length > 0 && children_count  == 0) return "#fbffb5";
  if(d.repeated && d.repeated.length > 0) return "yellow";
